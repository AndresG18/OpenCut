...
use decode::{FakeDecoder, FakeDecoderConfig, PixelFormat, Source, SourceId, SourceStreamId};
use render::{
    Affine, Blend, Crop, Node, NodeId, Opacity, Output, OutputFormat, RenderPlan, Renderer,
    SourceRef, readback_rgba8,
};
...
fn two_stacked_nodes_blend_with_half_opacity() {
    ...
    impl decode::Decoder for TwoDecoder {
        ...
        fn streams(
            ...
            let (is_top, inner) = self
                .mapping
                .get(&source.raw())
                .ok_or(decode::DecodeError::SourceNotFound)?;
            if *is_top {
                self.top.streams(*inner)
            } else {
                self.bottom.streams(*inner)
            }
        }
        ...
        fn read_frame(
            ...
            let (is_top, inner) = *self
                ...
                .get(&source.raw())
                .ok_or(decode::DecodeError::SourceNotFound)?;
            let mut frame = if is_top {
                self.top.read_frame(inner, stream, time)?
            } else {
                self.bottom.read_frame(inner, stream, time)?
            };
            frame.source = source;
            Ok(frame)
        }
        ...
        fn close(&mut self, source: decode::SourceId) -> Result<(), decode::DecodeError> {
            let (is_top, inner) = self
                ...
                .ok_or(decode::DecodeError::SourceNotFound)?;
            if is_top {
                self.top.close(inner)
            } else {
                self.bottom.close(inner)
            }
        }
    }
...
fn cropped_node_renders_only_cropped_region() {
    ...
    let plan = RenderPlan {
        ...
        nodes: vec![Node {
            ...
            // Crop coordinates are in source space. Keep the source-to-output
            // transform at identity so only the lower-right crop is drawn.
            transform: fill_affine(8, 8, 8, 8),
            opacity: Opacity::new(1.0).unwrap(),
            blend: Blend::SourceOver,
        }],
    ...
    let corner: [u8; 4] = pixels[0..4].try_into().unwrap();
