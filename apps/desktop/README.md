...
- **macOS**: Full Xcode (not only the standalone command line tools). Select it
  with `sudo xcode-select -s /Applications/Xcode.app/Contents/Developer`. On
  Xcode 16.3 and newer, install the separately distributed Metal compiler with
  `xcodebuild -downloadComponent MetalToolchain` before the first build.
